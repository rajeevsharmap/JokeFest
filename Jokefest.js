let jokeCounter = document.querySelector('.jokeCounter');
let statsMessage = document.querySelector('.JokeStatsMessage');
let count =0;

jokeCounter.innerText= count;
statsMessage.innerText = "🎯Ready!";
...
document.querySelector('.joke-request').addEventListener('click', async () => {
    ...
    const checkedCategories = [...document.querySelectorAll('.jokeCategories input[type="checkbox"]:checked')]
        .map(cb => cb.name);
        const categoryString = checkedCategories.length > 0
        ? checkedCategories.map(c => checkedCategories[c] || c).join(',')
        : 'Any';

    const checkedTypes = [...document.querySelectorAll('.jokeTypes input[type="checkbox"]:checked')]
        .map(cb => cb.name);

    let typeParam = '';
    if (checkedTypes.length === 1) {
        typeParam = `?type=${checkedTypes[0]}`;
    }
    const apiURL = `https://v2.jokeapi.dev/joke/${categoryString}${typeParam}`;
    ...
    try {
        ...
        if(count <= 5){
            statsMessage.innerText= "🔥 Warming Up!";
        }else if(count < 10){
            statsMessage.innerText= "😂 On Fire!";
        }else{
            statsMessage.innerText= "🏆 Joke Master!";
        }
        ...
        let like =document.querySelector('.JokeLike');
        let likeChanger =document.querySelector('.likeChange');

        like.addEventListener('click', ()=>{
            if(likeChanger.innerText == "LIKED"){
                likeChanger.innerText = "LIKE";
            }else{
                likeChanger.innerText="LIKED";
            }
        });
    }
    catch (error) {
